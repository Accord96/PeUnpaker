...
struct Sig
{
    ...
    template <typename... Entries>
    static void* find(PVOID buf, const size_t size)
    {
        constexpr auto k_patternSize = (Entries::k_size + ...);

        const auto* pos = static_cast<const unsigned char*>((PVOID)buf);
        const auto* const end = static_cast<const unsigned char*>((PVOID)buf) + size - k_patternSize + 1;
        while (pos < end)
        {
